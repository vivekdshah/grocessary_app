        ...
        itemBuilder: (context, index) {
          ...
          if(orderDetails.variations != null && orderDetails.variations!.isNotEmpty) {
            for(Variation variation in orderDetails.variations!) {
              ...
              // variationText += '${variationText.endsWith('') ? '' : ', '}${variation.variationValues?.first.level}';
              for(VariationValue value in variation.variationValues!) {
                variationText += '${variationText.endsWith(' ') ? '' : '- '}${value.level}';
                if ((value.optionPrice ?? 0) > 0) {
                  variationText += ' \$${value.optionPrice?.toStringAsFixed(2)}';
                }
                variationText += '\n';
              }
              variationText += '';
              ...
              children: [Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                    children: [Row(children: [
                      ...
                      variationText != '' ? Text(
                        variationText, style: robotoRegular,
                      ) : const SizedBox(),

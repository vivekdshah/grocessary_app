...
import 'package:efood_kitchen/my_printer_plugin/lib/my_printer_plugin.dart';
...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    const platform = MethodChannel('samples.flutter.dev/MyChannel');
    final OrderRepo orderRepo = Get.find();

    Future<void> startPrinting(Response apiResponse) async {
      if (apiResponse.statusCode == 200) {
        platform.invokeMethod('print_receipt', {'data': apiResponse.body});
      }
    }

    Future<void> handleNotificationPrinting(RemoteMessage message) async {
      if (message.notification != null) {
        String body = message.notification!.body ?? "";
        int orderId = int.tryParse(body) ?? 0;
        // Fetch order details
        Response apiResponse = await orderRepo.getOrderDetails(orderId);
        startPrinting(apiResponse); // Pass the API response to startPrinting
      }
    }
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      handleNotificationPrinting(message);
...
Future<dynamic> myBackgroundMessageHandler(RemoteMessage message) async {
  ...
  final OrderRepo orderRepo = Get.put(OrderRepo(apiClient: Get.put(ApiClient(appBaseUrl: AppConstants.baseUrl, sharedPreferences: sharedPreferences))));
  ...
  if (message.notification != null) {
    ...
    // Fetch order details
    Response apiResponse = await orderRepo.getOrderDetails(orderId);
    MyPrinterPlugin.startPrinting(apiResponse);
  }
  ...
  //
  // // Update order status to 'done'
  // orderRepo.updateOrderStatus(int.parse(message.notification!.body!),
  //   'delivered');
